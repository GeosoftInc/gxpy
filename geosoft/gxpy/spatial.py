...
:class:`Group` instances. Groups are named and are available to a user in a Geosoft viewer, which allows spatials to
be turned on or off, modify the transparency, or be deleted.

2D views can only accept 2D spatials, while a 3D view can accept both 2D and 3D spatials. When a 2D spatial is placed
in a 3D view, the spatial is placed on a the active plane inside the 3D view

:Classes:

    ============================ =============================================================================
    :class:`Group`               base class for named rendering spatials in 2D and 3D views.
    :class:`Draw`                2D drawing spatial, handles 2D drawing to a view or plane in a 3D view
    :class:`Draw_3d`             3D grawing spatial for 3D objects placed in a 3d view
    :class:`Color_symbols_spatial` spatial for 2D symbols rendered based on data values
    :class:`Aggregate_spatial`     spatial that contains a :class:`geosoft.gxpy.agg.Aggregate_image` instance
...
.. note::
    ...
    `spatial drawing tests <https://github.com/GeosoftInc/gxpy/blob/master/geosoft/gxpy/tests/test_spatial.py>`_
...
class SpatialException(Exception):
    """
    Exceptions from :mod:`geosoft.gxpy.spatial`.
...
class Spatial:
    """
    Base class for spatial bojects.

    :param name:                object name
    :param coordinate_system:   coordinate system, default is None
    :param gxobj:               optional base gxapi object that can satisfy get_ipj() and get_extents()
    ...
    def __init__(self, name='_spatial_', coordinate_system=None, gxobj=None):
    ...
    def coordinate_system(self):
        """
        `geosoft.gxpy.coordinate_system.Coordinate_system` instance of the object.
    ...
    def coordinate_system(self, cs):
        ...
        if self._gxobj and hasattr(self._gxobj, 'set_ipj'):
            self._gxobj.set_ipj(gxcs.Coordinate_system(cs).gxipj)
        else:
            self._cs = cs
...
class Group:
    """
    Geosoft spatial class.

    :parameters:

        :view:              gxpy.View
        :name:              spatial name, default is "_".
        :plane:             plane number, or plane name if drawing to a 3D view.  Default is plane number 0.
        :view_lock:         True to lock the view for a single-stream drawing spatial.  Default is False.
        :unit_of_measure:   unit of measurement for data in this spatial, default is ''
        :spatial_3d:        True for a 3D drawing spatial, default assumes a 2D drawing spatial to a plane.
        :mode:              `APPEND` (default), `NEW` or `READ_ONLY`

    :Properties:

        :view:              the :class:`geosoft.gxpy.view.View` instance that contains this spatial
        :name:              the name of the spatial
        :unit_of_measure:   the unit of measurement (uom) for this data in this spatial
        :name_uom:          uom decorated spatial name as it appears in a view
        :extent:            extent of the spatial in view units
        :extent_map_cm:     extent of the spatial in map cm
    ...
    .. versionchanged:: 9.3.1 added mode=REPLACE and changed mode=NEW to always create a new unique spatial.
    """
                ...
                try:
                    ...
                    if self._new_meta:
                        ...
                        try:
                            ...
                            self.view.gxview.write_spatial_storage(self.number, "Geosoft_META", bf)
                        finally:
                            del bf
    ...
    def __str__(self):
        if self.view.is_3d and not self.spatial_3d:
    ...
    def __init__(self,
        ...
        try:
            if view.lock:
                raise SpatialException(_t('This view is locked by spatial {}.'.format(view.lock)))
            if view_lock:
                view.lock = name
        finally:
            _lock.release()

        self.spatial_3d = False
        if view.is_3d:
            self.spatial_3d = spatial_3d

            if not spatial_3d:
                # setup a 2D drawing plane for this 2D spatial
                if plane is None:
        ...
        if mode == REPLACE:
            if self.view.gxview.exist_spatial(name):
                self.view.delete_spatial(name)

        elif mode == NEW:
            # if the spatial exists, find a new unique spatial name
            if self.view.gxview.exist_spatial(name):
                self._name = gxu.unique_name(name, self.view.gxview.exist_spatial, separator='_')

        elif self.view.gxview.exist_spatial(self.name):
            spatial_number = self.view.gxview.find_spatial(self.name)
            if self.view.gxview.spatial_storage_exists(spatial_number, "Geosoft_META"):
                bf = self.view.gxview.read_spatial_storage(spatial_number, "Geosoft_META")
                if bf.size():
                    try:
        ...
        self._view.gxview.start_spatial(self.name, mode)
        self._open = True

    def close(self):
        """ Close the spatial, unlocks the view"""
        self._close()
    ...
    def guid(self):
        ...
        """
        sr = gxapi.str_ref()
        self.view.gxview.get_spatial_guid(self.number, sr)
        return sr.value

    @property
    def view(self):
        """view that contains this spatial."""
        return self._view
    ...
    def drawing_plane(self):
        """ drawing plane of this spatial, None for a spatial in a 2D view."""
        if self.view.is_3d:
    ...
    def unit_of_measure(self):
        """
        Unit of measure for scalar data contained in this spatial. This is only relevant
        for spatials that contain scalar data, such as a Colour_symbols_spatial. For
    ...
    @property
    def number(self):
        """spatial number in the view"""
        return self.view.gxview.find_spatial(self.name)
    ...
    def _extent(self, unit=UNIT_VIEW):
        ...
        self.view.gxview.get_spatial_extent(self.name, xmin, ymin, xmax, ymax, unit)
        return xmin.value, ymin.value, xmax.value, ymax.value

    @property
    def extent(self):
        """spatial extent as (xmin, ymin, xmax, ymax) in view units"""
        return self._extent(UNIT_VIEW)

    @property
    def visible(self):
        """True if spatial is visible, can be set."""
        return self.name in self.view.spatial_list_visible

    @visible.setter
    def visible(self, visibility):
        if self.visible != visibility:
            marked = self.view.spatial_list_marked
            self.view.gxview.mark_all_spatials(0)
            self.view.gxview.mark_spatial(self.name, 1)
            if visibility is True:
                self.view.gxview.hide_marked_spatials(0)
            else:
                self.view.gxview.hide_marked_spatials(1)
            self.view.gxview.mark_all_spatials(0)
            for g in marked:
                self.view.gxview.mark_spatial(g, 1)

    def extent_map_cm(self, extent=None):
        """
        Return an extent in map cm.

        :param extent: an extent in view units as a tuple (xmin, ymin, xmax, ymax), Default is the spatial extent.
    ...
    def locate(self, location, reference=REF_CENTER):
        """
        Locate the spatial relative to a point.
        ...
        area += location
        self.view.gxview.relocate_spatial(self.name,
    ...
    def gx_metadata(self):
        """
        The spatial metadata as a Geosoft `geosoft.gxpy.metadata.Metadata` instance. This metadata
        may contain standard Geosoft metadata, such as unit_of_measure for data contained in the spatial,
...
class Draw(Group):
    """
    Create (start) a drawing spatial for 2D drawing elements.

    On a 3D view, 2D drawing elements are placed on the default drawing plane.
    Drawing spatials will lock the view such that only one drawing spatial can be instantiated at a time.
    
    Use `with Draw() as spatial:` to ensure correct unlocking when complete.
    ...
    def spatial_opacity(self):
        ...
        self.view.gxview.get_spatial_transparency(self.name, fref)
        return fref.value

    @spatial_opacity.setter
    def spatial_opacity(self, op):
        self.view.gxview.set_spatial_transparency(self.name, min(max(float(op), 0.), 1.))
...
class Draw_3d(Draw):
    """
    Create a 3D drawing spatial within a 3D view.

    3D drawing spatials accept 3D drawing objects that can be created using methods of this class.
    2D objects can also be drawn to a 3D spatial and will be placed on the default drawing plane
    within the 3D view.
    ...
    def __init__(self,
        ...
        if not isinstance(view, gxv.View_3d):
            raise SpatialException(_t('View is not 3D'))

        kwargs['spatial_3d'] = True
    ...
    def render_backfaces(self):
        ...
        .. versionadded:: 9.2
        """
        return bool(self.view.gxview.get_3d_spatial_flags(self.number) & 0b1)

    @render_backfaces.setter
    def render_backfaces(self, setting):
        if not setting and self.render_backfaces:
            raise SpatialException(_t('Once backface rendering is on it cannot be turned off.'))
        if not self.render_backfaces:
            f3d = (self.view.gxview.get_3d_spatial_flags(self.number) & 0b11111110) | 0b1
            self.view.gxview.set_3d_spatial_flags(self.number, f3d)
    ...
    def polyline_3d(self, points, style=LINE3D_STYLE_LINE):
        ...
        points = _make_ppoint(points)
        if points.length < 2:
            raise SpatialException(_t('Need at least two points.'))
    ...
    def polydata_3d(self,
        ...
        **Example**
        ...
        .. code::
            ...
            import geosogt.gxpy.spatial as gxg
        ...
        for item in data:
            ...
            if render:
                ...
                if symbol == SYMBOL_3D_SPHERE:
                    ...
                    raise SpatialException(_t('Symbol type not implemented'))
    ...
    def _surface(self, faces, verticies, coordinate_system=None):
        ...
        if np.max(faces) > n_verticies or np.min(faces) < 0:
            raise SpatialException(_t('Faces refer to verticies out of range of verticies.'))
...
def surface_spatial_from_file(v3d, file_name, spatial_name=None, overwrite=False):
    """
    Create a 3D surface spatial from a surface dataset file.
    ...
    :param spatial_name:  spatial name, default is the base file name.
    :param overwrite:   True to overwrite existing spatial

    .. versionadded:: 9.3.1
    """

    if spatial_name is None:
        spatial_name = os.path.basename(file_name)
        spatial_name = os.path.splitext(spatial_name)[0]
    if v3d.has_spatial(spatial_name) and not overwrite:
        raise SpatialException(_t('Cannot overwrite exing spatial "{}"').format(spatial_name))

    v3d.gxview.draw_surface_3d_from_file(spatial_name, file_name)


def contour(view, spatial_name, grid_file_name):
    """
    Create a contour spatial from a grid file.  A default contour interval is determined from the grid.

    :param view:            `geosoft.gxpy.view.View` instance
    :param spatial_name:      name for the contour spatial
    :param grid_file_name:  Grid file name

    .. versionadded:: 9.3
    """

    with Draw(view, spatial_name) as g:
...
def legend_color_bar(view,
    ...
    :param spatial_name:          name for the color_bar spatial, overwrites spatial if it exists.
    ...
    # ensure spatial name is unique in the view
    while spatial_name in view.spatial_list:
        spatial_name += '_'
...
class Color:
    ...
    def int_value(self, color):
        if color < 0:
            raise SpatialException(_t('Invalid color integer {}, must be >= 0').format(color))
    ...
    def __init__(self, **kwargs):
        ...
        for k in kwargs:
            if k == 'color':
                ...
                raise SpatialException(_t('Invalid text definition parameter ({})'.format(k)))
    ...
    def __init__(self, **kwargs):
        ...
        for k in kwargs:
            if k == 'line_color':
                ...
                raise SpatialException(_t('Invalid pen parameter ({})'.format(k)))
...
class Color_symbols_spatial(Group):
    """
    ...
    :Constructors:
        ...
        ============ =======================================
        :func:`new`  create a new symbol spatial in a view
        :func:`open` open an existing symbol spatial in a view
        ============ =======================================
    ...
    def __init__(self, view, spatial_name, **kwargs):

        self._gxcsymb = None
        super().__init__(view, spatial_name, **kwargs)
    ...
    def new(cls,
            ...
            **kwargs):
        """
        Create a new color symbols spatial with color mapping. If the spatial exists a new unique name is
        constructed.

        :param view:            the view in which to place the spatial
        ...
        :param mode:            REPLACE (default) or NEW, which creates a new unique name if spatial exists
        :return:                :class:`Color_symbols_spatial` instance
    ...
    def open(cls,
             ...
             spatial_name):
        """
        Open an existing color symbols spatial.

        :param view:        view that contains the spatial
        :param spatial_name:  name of the spatial, which must be a color symbols spatial
        :return:            :class:`Color_symbols_spatial` instance

        .. versionadded:: 9.2
        """
        cs = cls(view, spatial_name, mode=READ_ONLY)
        spatial_number = view.gxview.find_spatial(spatial_name)
        cs._gxcsymb = view.gxview.get_col_symbol(spatial_number)
        return cs

    def color_map(self):
        """
        Return the :class:`geosoft.gxpy.spatial.Color_map` of a color symbol spatial.
        ...
        cmap = geosoft.gxpy.spatial.Color_map(itr)
...
class Aggregate_spatial(Group):
    """
    Aggregate spatial in a view
    
    :Constructors:

        ======== ================================
        `open()` open an existing aggregate spatial
        `new()`  create a new aggregate spatial
        ======== ================================

    :Properties:
    
        :name:  aggregate spatial name
    ...
    def __init__(self, view, spatial_name, mode):

        self.agg = None
        super().__init__(view, spatial_name, mode=mode)

    @classmethod
    def new(cls, view, agg, name=None, mode=REPLACE):
        """
        Create a new aggregate spatial in a view.

        :param view:    `geosoft.gxpy.view.View` or `geosoft.gxpy.view.View_3d` instance
        :param agg:     `geosoft.gxpy.agg.Aggregate` instance.
        :param name:    spatial name, default is the aggregate name
        :param mode:    REPLACE (default) or NEW, which creates a unique name if the spatial exists
        ...
        if name is None:
            name = agg.name
        agg_spatial = cls(view, name, mode=mode)
        agg_spatial.agg = agg
        view.gxview.aggregate(agg.gxagg, agg_spatial.name)
        return agg_spatial
    ...
    def open(cls,
             ...
             spatial_name):
        """
        Open an existing aggregate spatial in a view.

        :param view:        `geosoft.gxpy.view.View` or `geosoft.gxpy.view.View_3d` instance
        :param spatial_name:  spatial name (or number)

        .. versionadded:: 9.2
        """
        agg_spatial = cls(view, spatial_name, mode=READ_ONLY)
        if isinstance(spatial_name, int):
            spatial_number = spatial_name
        else:
            spatial_number = view.gxview.find_spatial(agg_spatial.name)
        agg_spatial.agg = gxagg.Aggregate_image.open(view.gxview.get_aggregate(spatial_number))
        return agg_spatial


class VoxDisplayGroup(Group):
    """
    Vox display spatial in a view.  Use class methods `new()` and `open()`
    to create instances of `VoxDisplayGroup`.

    :Constructors:

        ======== ==================================
        `open()` open an existing vox_display spatial
        `new()`  create a new vox_display spatial
        ======== ==================================
   ...
   .. versionadded:: 9.3.1
    ...
    def __init__(self, view3d, spatial_name, mode=REPLACE):

        self._voxd = None
        if not view3d.is_3d:
            raise SpatialException(_t('View must be 3d'))
        super().__init__(view3d, spatial_name, mode=mode)

    @classmethod
    def new(cls, view3d, voxd, name=None, mode=REPLACE):
        """
        Add a VoxDisplay as a new spatial in the view

        :param view3d:  `geosoft.gxpy.view.View_3d` instance
        :param voxd:    `geosoft.gxpy.vox_display.VoxDisplay` instance
        :param name:    spatial name, default is the voxd name
        :param mode:    REPLACE (default) or NEW, which creates a unique name if the spatial exists
        ...
        if name is None:
            name = voxd.name
        voxd_spatial = cls(view3d, name, mode=mode)
        if voxd.is_vector:
            ...
            view3d.gxview.voxd(voxd.gxvoxd, voxd_spatial.name)
        voxd_spatial._voxd = voxd
        voxd_spatial.unit_of_measure = voxd.unit_of_measure
        return voxd_spatial
    ...
    def open(cls,
        ...
        :param spatial_name:  the name of the spatial to open, must be a `gxapi.GXVOXD` or
                            `gxapi.GXVECTOR3D`.

        .. versionadded: 9.3.1
        """
        voxd_spatial = cls(view, spatial_name, mode=READ_ONLY)
        if view.gxview.is_spatial(spatial_name, gxapi.MVIEW_IS_VOXD):
            voxd_spatial._voxd = gxvoxd.VoxDisplay.gxapi_gxvoxd(voxd_spatial.view.gxview.get_voxd(voxd_spatial.number))
        elif view.gxview.is_spatial(spatial_name, gxapi.MVIEW_IS_VECTOR3D):
            voxd_spatial._voxd = gxvoxd.VoxDisplay.gxapi_gxvoxd(voxd_spatial.view.gxview.get_vector_3d(voxd_spatial.number),
                                                              name=spatial_name + ".geosoft_vectorvoxel")
        else:
            raise SpatialException('Group "{}" is not a GXVOXD or a GXVECTOR3D'.format(spatial_name))
        return voxd_spatial

    @property
    def voxd(self):
        """
        The `geosoft.gxpy.vox_display.VoxDisplay` for this vox spatial.
...
class Color_map:
    ...
    def set_linear(self, minimum, maximum, inner_limits=True, contour_interval=None):
        ...
        if inner_limits:
            if self.length < 3:
                raise SpatialException(_t("Colour map must have length >= 3 for inner edge linear range."))
